...
| [SPIKE_02](SPIKE_02_TERMINAL_CONTAINMENT.md) | SPIKE | APPROVED_FOR_IMPLEMENTATION | N/A (SPIKE) | NOT_PERFORMED |
...
| [INTEGRATION_01](INTEGRATION_01_ROBUSTNESS_HARDENING.md) | INTEGRATION_HARDENING | APPROVED_FOR_IMPLEMENTATION | IMPLEMENTING | NOT_PERFORMED |
...
> SPIKE 任务书的 `Win7-Validation` 元数据保持正式合同值 `NOT_PERFORMED`；MVP 独立实测结果见 `validation/README.md`、`docs/STATUS.md` 和 `docs/status/latest-validation.json`，不会把 MVP 证据冒充正式任务 Gate。
