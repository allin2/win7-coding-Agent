...
## 20. 需求修订登记（2026-09-24，负责人授权按推荐修订）

以下修订以追加方式登记，不改写 §3/§4 原条文，WIN7-36 按原条文取得的结论不变；实施见
[A9-19](A9_19_LIVE_PROGRESS_AND_WORKBENCH_LAYOUT.md)。

- §3 末段“不把模型 token 级输出纳入范围”修订为：模型输出以内存预览实时呈现，纳入 Alpha 2
  （A9-19 P02，ADR-0135）；交互式终端与 stdin 注入仍不纳入。
- §3/§4 验收补充时效断言：过程可见须以“落盘 → DOM 可见”时刻验收，不再只断言完成后的顺序（A9-19 P06）。
- §4 U02 补充：对话行必须可见标题；行数达标但标题被挤占不算通过（A9-19 L01，缘由为 WIN7-36 实机截图中
  全部对话行只显示“状态 · 时间”）。
- §7“Review 入口继续 disabled + fail-closed 文案”修订为：Review 暂缓期间左栏不展示 Review 页签，权限对话框
  继续 fail-closed 说明未开放（A9-19 L05）；R01 不静默降级的要求不变。
