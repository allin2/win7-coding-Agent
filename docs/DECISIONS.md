...
## ADR-0024 原型审查轮 1 语义裁决：CLI 退出码、Replay 一致性模型与 Policy DENY 语义

- 状态：Accepted（2026-07-27，架构师审查轮 1 裁决；仅适用于 prototype/full-agent-skeleton 分支，正式阶段吸收须重新评审）
- 背景：原型实现终点 `31d1bda` 的独立审查结论为 `KEEP_READY_FOR_PROTOTYPE_REVIEW_AND_REPAIR`。审查暴露三处任务书语义未冻结导致的实现分歧：① argparse 默认参数错误退出码 2 与 CANCELLED 的退出码 2 冲突，Replay 初始化失败的退出码归类不明；② ReplayProvider 以可写方式 `sqlite3.connect` 打开事件库（不存在路径会创建空文件），且只重放响应序列、不验证当前请求与录制请求是否一致，重放结论可能建立在完全不同的请求之上；③ PolicyEngine DENY 被记为 `tool.result` 事件，且 `NoPolicyViolationRule` 把 DENY 本身当作违规证据，语义上把"安全策略成功拦截"与"策略被违反"混为一谈。
- 决策：（1）**CLI 退出码冻结**：`0` = COMPLETED；`1` = Run 失败（FAILED，含运行期 `REPLAY_MISMATCH`、`EVENT_STORE_FAILED` 导致的失败）；`2` = Run 取消（CANCELLED）；`3` = 参数、配置、Replay 初始化及 CLI 基础设施错误（argparse 错误经自定义 `error()` 归入 3，`ProviderError`、WorkspaceError、事件库无法创建等 Run 尚未启动的错误同归 3）。（2）**Replay 一致性模型**：事件库必须以 SQLite URI `file:<path>?mode=ro` 只读打开，不存在的路径不得创建文件；Schema、Run 或录制序列错误转为 `ProviderError`，CLI 返回 3；录制时每个 `model.request` 事件保存规范化 SHA-256 请求指纹（`request_fingerprint`：对消息序列、工具名清单与 turn 的确定性 JSON 序列化取 `hashlib.sha256`）；重放时验证 turn、请求指纹与请求响应配对，请求不一致、响应缺失、顺序错误或录制耗尽均为 `REPLAY_MISMATCH`（Run → FAILED）；指纹机制只防意外不一致，**不承诺抵抗攻击者协调修改数据库和指纹**（威胁模型声明记入 INTERFACE_RISKS）。（3）**Policy DENY 语义**：DENY 表示安全策略成功拦截，不是 policy violation；DENY 后工具实现绝对不能执行（实现函数调用次数为 0）；事件使用新增的 `tool.denied`，不使用正常 `tool.result` 表示执行（`tool.result` 仅保留给真实执行）；拒绝结果可以作为结构化观察返回模型，Run 可以继续寻求只读替代方案；`NoPolicyViolationRule` 只拒绝"没有匹配 ALLOW 却实际执行"的情况。（4）事件类型表同步新增 `state.transition_rejected`（非法转换审计）；事件类型冻结清单以修订后的任务书 §4.7 为准。（5）流程记录：实现方自行将 Phase-Gate 更新为 `READY_FOR_PROTOTYPE_REVIEW` 的偏差获架构师书面追认（任务书 §0.4），追认仅确认本次审查入口有效，不改变 Gate 必须由授权人修改的规则。
- 后果：退出码 2/3 语义不再撞车，运维脚本可稳定分支；Replay 从"响应顺序重放"升级为"请求-响应一致性验证"，篡改或环境漂移可被检出，代价是事件库新增指纹字段（schema_version 仍为 1，属 payload 字段扩展）；DENY 语义修正后，P12 类测试与验证规则的断言需同步改写；`tool.denied`/`state.transition_rejected` 使事件轨迹能区分"拦截"与"执行"，Phase 6 正式 Agent Loop 设计时应沿用该区分。
