...
exports.validateTrustedShellEnvironmentOverlay = exports.createTrustedShellEnvironment = exports.isForbiddenTrustedShellEnvironmentName = void 0;
...
function isForbiddenTrustedShellEnvironmentName(name) {
    const upper = name.toUpperCase();
    if (FORBIDDEN_ENVIRONMENT_EXACT.has(upper) || upper.startsWith('ELECTRON_') || upper.startsWith('NODE_'))
        return true;
    if (['API_KEY', 'APIKEY', 'ACCESS_KEY', 'ACCESSKEY', 'PRIVATE_KEY', 'PRIVATEKEY',
        'CREDENTIAL', 'AUTHORIZATION', 'PASSWORD', 'SECRET', 'TOKEN'].some((fragment) => upper.includes(fragment)))
        return true;
    return /(?:PROVIDER|OPENAI|ANTHROPIC|DEEPSEEK|AZURE).*KEY/.test(upper);
}
exports.isForbiddenTrustedShellEnvironmentName = isForbiddenTrustedShellEnvironmentName;
function createTrustedShellEnvironment(inherited, overlay, sensitiveValues = []) {
...
exports.createTrustedShellEnvironment = createTrustedShellEnvironment;
/**
 ...
 */
function validateTrustedShellEnvironmentOverlay(overlay, sensitiveValues = []) {
    const output = Object.create(null);
    let totalCharacters = 0;
    const keys = Object.keys(overlay);
    if (keys.length > 64)
        throw new Error('A9_ENV_OVERLAY_REJECTED: too many entries');
    for (const key of keys) {
        ...
        if (totalCharacters > 32767)
            throw new Error('A9_ENV_OVERLAY_REJECTED: size limit exceeded');
        output[key] = value;
    }
    return output;
}
exports.validateTrustedShellEnvironmentOverlay = validateTrustedShellEnvironmentOverlay;
...
function containsKnownEnvironmentSecret(value, secrets) {
    const decode = (text) => text.replace(/(?:%[0-9a-f]{2})+/gi, (run) => Buffer.from(run.slice(1).split('%').map((hex) => parseInt(hex, 16))).toString('utf8'));
...
//# sourceMappingURL=trusted-shell-environment.js.map