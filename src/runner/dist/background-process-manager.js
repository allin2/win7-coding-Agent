...
function backgroundSensitiveVariants(secret) {
...
class BackgroundProcessManager {
    constructor(helperTransport, redactText = (text) => text, getSensitiveValues = () => [], onStateChange = () => { }) {
    ...
    containAfterStatePersistenceFailure(handle, error) {
     ...
     */
    async start(handleId, command, shellExe, shellArgs, cwd, environment, helperRequest) {
        ...
        if (helperRequest)
            return this.startViaHelper(handleId, command, cwd, helperRequest, environment);
        let child;
        try {
            child = (0, child_process_1.spawn)(shellExe, shellArgs, {
                cwd,
        ...
        const entry = {
            ...
            logDecoders: { stdout: new string_decoder_1.StringDecoder('utf8'), stderr: new string_decoder_1.StringDecoder('utf8') },
        };
        ...
        child.on('error', (_err) => {
            ...
            if (persistenceError)
                this.containAfterStatePersistenceFailure(handle, persistenceError);
        });
        child.on('close', (code) => {
    ...
    async startViaHelper(handleId, command, cwd, request, environment) {
        ...
        try {
            ...
            if (handle.status === 'failed')
                throw new Error(handle.logs.stderr.join('\n') || 'helper failed after readiness');
            ...
            const cleanupConfirmed = completion.kind === 'response'
                ? completion.response.type === 'execution_result'
                    && (0, native_protocol_1.hasCompleteHelperCleanupProof)(completion.response)
            ...
            const persistenceError = this.notifyStateChange(handle);
            if (persistenceError)
                this.appendRedactedText(handle, 'stderr', `managed process state persistence failed: ${persistenceError.message}`);
    ...
    finishHelperInvocation(entry, result) {
        ...
        if (result.kind !== 'response' || result.response.type !== 'execution_result') {
            ...
            this.appendLog(entry, 'stderr', Buffer.from(detail, 'utf8'));
            this.flushPendingLog(entry, 'stderr');
            this.notifyStateChange(handle);
            return;
        }
        const response = result.response;
        entry.helperCleanupConfirmed = (0, native_protocol_1.hasCompleteHelperCleanupProof)(response);
        handle.cleanupRequired = !entry.helperCleanupConfirmed;
        try {
            this.appendLog(entry, 'stdout', (0, native_protocol_1.decodeNativeHelperBase64)(response.stdoutBase64, response.stdoutSize, 'stdoutBase64'));
            this.appendLog(entry, 'stderr', (0, native_protocol_1.decodeNativeHelperBase64)(response.stderrBase64, response.stderrSize, 'stderrBase64'));
        }
        catch (error) {
        ...
        if (safeLength > 0 && variants.length > 0) {
            ...
            for (const variant of variants) {
                for (let size = Math.min(variant.length - 1, prefix.length); size > overlap; size -= 1) {
                    if (prefix.endsWith(variant.slice(0, size))) {
                        overlap = size;
                        break;
                    }
                }
    ...
    appendRedactedText(handle, stream, raw) {
        if (!raw)
            return;
        ...
        for (const line of text.split('\n')) {
            ...
            if (target.length > MAX_LOG_LINES) {
                target.shift();
                if (stream === 'stdout')
                    handle.droppedLogLines.stdout += 1;
                else
                    handle.droppedLogLines.stderr += 1;
            }
        }
    }
    /**
    ...
    async stop(handleId) {
        ...
        if (entry.recoveredFact && handle.cleanupRequired === true) {
            throw Object.assign(new Error(`后台进程 ${handleId} 是重启前遗留的未确认 D-013 清理事实；应用不会把 helper PID 消失误判为进程树已清理。请完成独立残留检查。`), { code: 'A9_MANAGED_PROCESS_CLEANUP_UNCONFIRMED' });
        }
        ...
        if (handle.status === 'running' || handle.status === 'starting') {
            if (entry.recoveredFact) {
                ...
                const error = new Error(`后台进程 ${handleId} (PID ${handle.pid}) 是重启后恢复的 PID 事实，无法证明仍是原进程；为避免 PID 复用误杀，应用不会发送停止信号。请在系统中核对并停止后再次点击 Stop。`);
                error.code = 'A9_RECOVERED_PROCESS_IDENTITY_UNCONFIRMED';
                throw error;
            }
            const targetPid = child?.pid ?? handle.pid;
            if (targetPid) {
                const kill = await (0, process_cleanup_1.killProcessTree)(targetPid);
                if (!kill.success) {
     ...
     */
    adoptRecoveredFact(handleId, fact) {
        if (this.processes.has(handleId)) {
            throw new Error(`后台进程句柄已存在: ${handleId}`);
        }
    ...
    requiresCleanupAttention(entry) {
        ...
        if (options.stopManaged) {
            for (const [id, entry] of this.processes.entries()) {
                ...
                if (entry.handle.cleanupRequired === true && entry.recoveredFact) {
                    ...
                    const kill = await (0, process_cleanup_1.killProcessTree)(entry.child.pid);
                    if (kill.success) {
...
BackgroundProcessManager.MAX_PROCESSES = 3;
