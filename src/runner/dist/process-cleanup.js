...
exports.killProcessTree = exports.parseWindowsProcessTable = void 0;
...
function parseWindowsProcessTable(output, rootPid) {
    ...
    for (const line of String(output).split(/\r?\n/)) {
        ...
        if (!Number.isInteger(parentPid) || !Number.isInteger(processId))
            continue;
...
exports.parseWindowsProcessTable = parseWindowsProcessTable;
function enumerateWindowsDescendants(pid) {
    return new Promise((resolve) => {
        childProcess.execFile('wmic', ['process', 'get', 'ParentProcessId,ProcessId', '/format:csv'], { windowsHide: true, maxBuffer: 4 * 1024 * 1024 }, (error, stdout) => resolve(error ? null : parseWindowsProcessTable(String(stdout), pid)));
    });
}
/**
...
async function killProcessTree(pid) {
    if (!pid || pid <= 0)
        return { success: false, error: 'Invalid PID', method: 'posix_signal' };
