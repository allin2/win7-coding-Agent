...
describe('R4: provider config persistence and DPAPI integration', () => {
  ...
  it('model switch rebuilds the loop while preserving tool results and user context', async () => {
    const fixture = await startFixtureModel();
    let secondModelSawPriorToolResults = false;
    let messagesSeen: any[] = [];
    const spyServer = await new Promise<{ close: () => Promise<void> }>((resolve) => {
      // 包装 fixture：记录第二次配置后收到的 messages。
      void fixture;
      resolve({ close: () => Promise.resolve() });
    });
    // 使用带记录的专用 fixture。
    const recording = await new Promise<{ baseUrl: string; close: () => Promise<void> }>((resolve) => {
      const server = http.createServer((req, res) => {
        let body = '';
        req.on('data', (d: Buffer) => { body += d.toString('utf8'); });
        req.on('end', () => {
          const parsed = JSON.parse(body);
          messagesSeen = parsed.messages ?? [];
          res.writeHead(200, { 'Content-Type': 'text/event-stream' });
          if (parsed?.tools?.[0]?.function?.name === 'probe_test_echo') {
            ...
            if (toolMsgs.length === 0) {
              ...
              sse(res, { choices: [{ delta: { content: 'continued with full history' }, finish_reason: 'stop' }] });
            }
          }
          res.write('data: [DONE]\n\n');
          res.end();
    ...
    try {
      await spyServer.close();
      const runtime = makeRuntime({ safeStorage: fakeSafeStorage(), vaultPlatform: 'win32' });
      runtime.setMode('full_access');
      await runtime.configureProvider({ baseUrl: recording.baseUrl, model: 'model-a' });
      fs.writeFileSync(path.join(env.workspaceRoot, 'note.txt'), 'hello\n');
      const firstTurn = await runtime.submitTurn('read the note');
      expect(firstTurn.ok).toBe(true);

      // 模型切换：重建 loop 但保留 assistant tool_calls 与 tool results。
      const switched = await runtime.configureProvider({ baseUrl: recording.baseUrl, model: 'model-b' });
      expect(switched.model).toBe('model-b');
      const secondTurn = await runtime.submitTurn('continue');
      expect(secondTurn.ok).toBe(true);
      const toolMessages = messagesSeen.filter((m: any) => m.role === 'tool');
      const assistantWithTools = messagesSeen.filter((m: any) => m.role === 'assistant' && Array.isArray(m.tool_calls));
      secondModelSawPriorToolResults = toolMessages.length > 0 && assistantWithTools.length > 0;
      expect(secondModelSawPriorToolResults).toBe(true);
      expect(messagesSeen.some((m: any) => m.role === 'user')).toBe(true);
      runtime.shutdown();
    } finally {
