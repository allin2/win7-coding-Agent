...
function verifyArtifactHashes(product, evidence) {
  const expectedFiles = {
    ...
    renderer_runner_log: path.join(product, 'renderer/runner-log.js'),
  };
  if (!evidence || !evidence.artifact_hashes || typeof evidence.artifact_hashes !== 'object') {
    throw new Error('A8_BOUNDARY_ARTIFACT_HASHES_MISSING');
  }
  Object.entries(expectedFiles).forEach(([name, filePath]) => {
    ...
    if (evidence.artifact_hashes[name] !== expected) throw new Error(`A8_BOUNDARY_ARTIFACT_HASH_MISMATCH:${name}`);
  });
}
...
function main() {
  ...
  if (binding) {
    if (electron !== path.join(binding.candidateRoot, 'electron.exe')) throw new Error('A8_BOUNDARY_RUNTIME_ROOT_MISMATCH');
    if (product !== path.join(binding.candidateRoot, 'resources', 'app', 'product')) throw new Error('A8_BOUNDARY_PRODUCT_ROOT_MISMATCH');
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'win7-a8-boundary-smoke-'));
  ...
  const args = [
    product,
    '--mvp-id=A8-04-BOUNDARY',
    `--a8-boundary-smoke-report=${report}`,
    `--a8-boundary-smoke-screenshot=${screenshot}`,
  ...
  if (!fs.existsSync(report)) throw new Error('A8_BOUNDARY_REPORT_MISSING');
  if (!fs.existsSync(screenshot)) throw new Error('A8_BOUNDARY_SCREENSHOT_MISSING');
  const evidence = JSON.parse(fs.readFileSync(report, 'utf8'));
  if (evidence.status !== 'PASS') throw new Error(`A8_BOUNDARY_REPORT_${evidence.status || 'INVALID'}`);
  ...
  if (!Array.isArray(evidence.cases) || evidence.cases.length !== 8 || evidence.cases.some((item) => item.status !== 'PASS')) {
    throw new Error('A8_BOUNDARY_CASE_MATRIX_INVALID');
  }
  if (fs.statSync(screenshot).size < 1024) throw new Error('A8_BOUNDARY_SCREENSHOT_INVALID');
  ...
  const finalized = finalizeEvidenceReport(evidence, {
    ...
    developerCandidateId: 'A8-04-DEVELOPER-SOURCE-TREE',
    expectedCaseIds: [
      'A8T-01-TERMINAL-DISABLED',
      'A8B-01-BROWSER-DISABLED',
      'A8R-09-RUNNER-LOG-BOUNDARY',
      'A8C-04-IPC-TERMINAL-FAIL-CLOSED',
    ],
  });
...
try {
  ...
  process.stderr.write(`A8_BOUNDARY_SMOKE_RUNNER_FAILED:${error && error.stack ? error.stack : error}\n`);
  process.exitCode = 1;
}
