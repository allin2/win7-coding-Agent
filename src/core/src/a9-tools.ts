    ...
    },
    {
      ...
      inputSchema: {
        properties: {
          ...
          startLine: {
            type: 'number',
            description: '起始行号（正整数、1-based，默认 1；大文件同样支持分段读取）。',
            default: 1,
            minimum: 1,
          },
          maxLines: {
            type: 'number',
            description: '最大读取行数（正整数、默认 200，最多 2000）；预览最多 128 KiB，长行截断会明确标记。',
            default: 200,
            minimum: 1,
            maximum: 2000,
          },
          encoding: {
            type: 'string',
            description: '省略则自动探测 BOM/UTF-8/GBK；显式 utf-8、gbk、utf-16le 优先。无 BOM UTF-16LE 请显式指定；binary 仅返回元数据。',
