...
export function buildA9SystemPrompt(options: SystemPromptOptions = {}): SystemPromptContract {
  ...
  const content = [
    ...
    '- At key moments (starting a step, significant findings, direction changes, blockers, entering verification), briefly tell the user what you are doing and why in one or two sentences.',
    '- Such notes are descriptive only: they never substitute for actual execution or evidence.',
    '- Never fabricate progress; if there is nothing to report, stay silent and keep working.',
    '',
