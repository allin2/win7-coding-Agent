...
try {
  ...
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'a7-rc0708-kit-'));
  ...
  if (!/^RC0708_WINDOWS_LIFECYCLE_KIT_[A-Za-z0-9._-]+\.zip$/.test(lock.output_filename || '')) {
    throw new Error('LIFECYCLE_KIT_OUTPUT_FILENAME_INVALID');
  }
  ...
  const result = {
    schema_version: 1,
    status: 'RC0708_LIFECYCLE_KIT_BUILD_PASS',
    source_commit: head,
    target_candidate_sha256: lock.candidate.sha256,
  ...
  writeJson(path.join(outputRoot, 'rc0708-kit-build-result.json'), result);
  fs.rmSync(temporary, { recursive: true, force: true });
  process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
} catch (error) {
  process.stderr.write(`RC0708_KIT_BUILD_FAILED:${error && error.message ? error.message : String(error)}\n`);
  process.exitCode = 1;
}
