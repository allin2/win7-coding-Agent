A7 RC-05 / RC-06 Windows validation kit
...
directories. Keep the validation kit outside the product root.

  certutil -hashfile C:\path\Win7CodingAgent-0.1.0-rc.1-win7-x64.zip SHA256
  certutil -hashfile C:\path\RC0506_WINDOWS_VALIDATION_KIT_20260813.zip SHA256
...
Return the complete evidence directory, command transcript, both ZIP dual hashes and
the final tasklist residue snapshot. A harness PASS remains a Windows pre-lease result;
the Win7 cases must be repeated under a new signed RC lease.
