...
const LOCK_FILE = 'a9-16-win7-34-input-lock.json';
...
const REQUIRED_FILES = [
  ...
  KIT_FILE, 'A9_16_WIN7_34_VALIDATION.md', 'RUN_A9_16_W34_INTEGRITY.cmd', 'RUN_WIN7_34_REPORT_VERIFY.cmd',
  'validation/a9-package-integrity-w34.cjs', 'validation/a9-win7-34-report.cjs',
  'validation/a9-win7-34-smoke.cjs', 'validation/a9-win7-34-driver.cjs', 'validation/a9-projection-contract.cjs',
];
...
function selectPhysicalFileSystem(runtimeVersions = process.versions, loadOriginalFs = () => require('original-fs')) {
  ...
  for (const method of ['existsSync', 'realpathSync', 'readdirSync', 'readFileSync', 'statSync']) {
    if (!physical || typeof physical[method] !== 'function') throw new Error(`A9_W34_PHYSICAL_FILESYSTEM_UNAVAILABLE:${method}`);
  }
  return physical;
}
...
function listFiles(root, current = root, out = [], fileSystem = fs) {
  for (const entry of fileSystem.readdirSync(current, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, 'en'))) {
    ...
    if (entry.isSymbolicLink()) throw new Error(`A9_W34_PACKAGE_SYMLINK_PROHIBITED:${absolute}`);
    ...
    else throw new Error(`A9_W34_PACKAGE_SPECIAL_FILE_PROHIBITED:${absolute}`);
  }
  return out;
}
...
function externalFile(value, candidateRoot, label, fileSystem) {
  if (!value) throw new Error(`A9_W34_${label}_REQUIRED`);
  const unresolved = path.resolve(value);
  if (!fileSystem.existsSync(unresolved) || !fileSystem.statSync(unresolved).isFile()) throw new Error(`A9_W34_${label}_NOT_FILE`);
  const resolved = fileSystem.realpathSync(unresolved);
  if (contained(candidateRoot, resolved)) throw new Error(`A9_W34_${label}_INSIDE_CANDIDATE`);
  return resolved;
}

function verifyFullTree(candidateRoot, manifest, fileSystem) {
  if (!Array.isArray(manifest.files) || !manifest.files.length) throw new Error('A9_W34_MANIFEST_FILES_INVALID');
  const expected = new Map(manifest.files.map((item) => [item.path, item]));
  if (expected.size !== manifest.files.length) throw new Error('A9_W34_MANIFEST_DUPLICATE_PATH');
  const actual = listFiles(candidateRoot, candidateRoot, [], fileSystem).filter((item) => item !== 'release-manifest.json');
  if (actual.length !== expected.size) throw new Error(`A9_W34_FILE_COUNT_MISMATCH:${actual.length}:${expected.size}`);
  for (const relative of actual) {
    ...
    if (!item || !Number.isSafeInteger(item.size) || item.size < 0 || !/^[a-f0-9]{64}$/.test(item.sha256 || '')) {
      throw new Error(`A9_W34_MANIFEST_ENTRY_INVALID:${relative}`);
    }
    if (fileSystem.statSync(absolute).size !== item.size || fileHash(absolute, fileSystem) !== item.sha256) {
      throw new Error(`A9_W34_FILE_MISMATCH:${relative}`);
    }
  }
  return expected;
}
...
function verifyPe32PlusAmd64(filePath, expectDll, fileSystem) {
  ...
  if (bytes.length < 0x100 || bytes.readUInt16LE(0) !== 0x5a4d) throw new Error(`A9_W34_NATIVE_NOT_PE:${filePath}`);
...
function verifyLock(lock, manifest, registryBytes) {
  ...
  if (lock.schema_version !== 1 || lock.lock_id !== 'A9-16-INPUTS-RESPONSIVE-UI-WIN7-34'
    ...
    || lock.gates?.win7 !== 'NOT_PERFORMED_WIN7_34' || lock.gates?.alpha !== 'NOT_PERFORMED'
  ...
  if (approval?.path !== 'release/win7-product-v3/a9-v25-approved-kits.json'
    ...
    throw new Error('A9_W34_APPROVAL_REGISTRY_BINDING_INVALID');
  }
...
function verifyAuthority(candidateRoot, manifest, zipPath, authority, fileSystem) {
  ...
  if (path.basename(lockPath).toLowerCase() !== LOCK_FILE || path.basename(registryPath).toLowerCase() !== 'a9-v25-approved-kits.json') {
    throw new Error('A9_W34_EXTERNAL_AUTHORITY_FILENAME_INVALID');
  }
  ...
  if (!/^[a-f0-9]{64}$/.test(String(authority.releaseAuthoritySha256 || ''))
    || sha256(authorityBytes) !== authority.releaseAuthoritySha256) throw new Error('A9_W34_AUTHORITY_PIN_MISMATCH');
  if (!lockBytes.equals(fileSystem.readFileSync(path.join(candidateRoot, LOCK_FILE)))) {
    throw new Error('A9_W34_FORMAL_INPUT_LOCK_BYTE_MISMATCH');
  }
  const lock = readJson(lockPath, fileSystem, 'A9_W34_FORMAL_INPUT_LOCK_JSON_INVALID');
  const registry = readJson(registryPath, fileSystem, 'A9_W34_APPROVAL_REGISTRY_JSON_INVALID');
  const approved = readJson(authorityPath, fileSystem, 'A9_W34_RELEASE_AUTHORITY_JSON_INVALID');
  const { runner, storage, electron, approval } = verifyLock(lock, manifest, registryBytes);
  if (approved.schema_version !== 1 || approved.kind !== 'WIN7_34_RELEASE_AUTHORITY'
    ...
    throw new Error('A9_W34_RELEASE_AUTHORITY_BINDING_INVALID');
  }
  ...
  if (!found) throw new Error('A9_W34_NATIVE_BUILD_KIT_NOT_APPROVED');
  return {
...
function verifyZip(zipPath, manifest, manifestBytes, files, fileSystem) {
  if (path.basename(zipPath) !== PACKAGE_NAME) throw new Error('A9_W34_ZIP_FILENAME_INVALID');
  ...
  const fail = () => { throw new Error('A9_W34_CANDIDATE_ZIP_INVALID'); };
...
function verifyAcceptanceCandidate(zipPath, manifestPath, kitPath, authority, fileSystem = fs) {
  ...
  const manifest = readJson(manifestPath, fileSystem, 'A9_W34_MANIFEST_JSON_INVALID');
  if (manifest.schema_version !== 1 || manifest.release_id !== 'WIN7-CODING-AGENT-A9-ALPHA1'
    ...
    throw new Error('A9_W34_MANIFEST_IDENTITY_INVALID');
  }
  const candidateRoot = fileSystem.realpathSync(path.dirname(manifestPath));
  if (contained(candidateRoot, fileSystem.realpathSync(zipPath))) throw new Error('A9_W34_ZIP_INSIDE_CANDIDATE');
  const files = verifyFullTree(candidateRoot, manifest, fileSystem);
  for (const relative of REQUIRED_FILES) if (!files.has(relative)) throw new Error(`A9_W34_CLOSURE_MISSING:${relative}`);
  if (files.get(KIT_FILE)?.sha256 !== fileHash(kitPath, fileSystem)) throw new Error('A9_W34_KIT_MANIFEST_MISMATCH');
  const kit = readJson(kitPath, fileSystem, 'A9_W34_KIT_JSON_INVALID');
  if (kit.schema_version !== 1 || kit.kit_id !== 'A9-16-WIN7-34-RESPONSIVE-UI-20260915-01'
    || kit.candidate_label !== 'WIN7-34' || kit.candidate_id !== manifest.release_id
    ...
    || kit.required_cases.some((item) => !/^W34-1[0-9]-|^W34-0[1-9]-/.test(item?.case_id || '')
      || !Array.isArray(item.assertions) || !item.assertions.length)) {
    throw new Error('A9_W34_KIT_CONTRACT_INVALID');
  }
  const caseIds = new Set(kit.required_cases.map((item) => item.case_id));
  if (!caseIds.has('W34-03-INSPECTOR-PERSISTED-RESTART')
      || !caseIds.has('W34-04-APPROVAL-FAILURE-ORDER')
      || !caseIds.has('W34-09-LATEST-OUTCOME-PROJECTION')) {
    throw new Error('A9_W34_PROJECTION_CASES_MISSING');
  }
  // ADR-0130：A9-16 UI 子集用例必须真实存在，否则不得签发包完整性。
  for (const caseId of ['W34-11-LEFT-PANE-DENSITY-AND-CAPACITY', 'W34-12-DESKTOP-FOUR-STATE-DOM-KEEPALIVE',
    'W34-13-BREAKPOINT-CONVERGENCE-REGRESSION', 'W34-14-KEYBOARD-AND-FOCUS-CONTRACT',
    'W34-15-REAL-125-PERCENT-DPI-LAYOUT']) {
    if (!caseIds.has(caseId)) throw new Error(`A9_W34_UI_SUBSET_CASE_MISSING:${caseId}`);
  }
  ...
  if (manifest.locked_inputs?.electron_zip?.sha256 !== electron.sha256
    ...
    || manifest.required_native?.electron_abi !== 110) throw new Error('A9_W34_NATIVE_BINDING_INVALID');
  if (manifest.release_authority?.formal_input_lock_sha256 !== resolvedAuthority.authorityIdentity.formal_input_lock_sha256
    ...
    throw new Error('A9_W34_MANIFEST_AUTHORITY_BINDING_INVALID');
  }
  ...
  const runtime = readJson(path.join(candidateRoot, 'resources', 'app', 'a9-runtime.json'), fileSystem, 'A9_W34_RUNTIME_JSON_INVALID');
  const packageJson = readJson(path.join(candidateRoot, 'resources', 'app', 'package.json'), fileSystem, 'A9_W34_PACKAGE_JSON_INVALID');
  if (runtime.release_id !== manifest.release_id || runtime.version !== manifest.version || runtime.state_schema !== 4
    ...
    throw new Error('A9_W34_RUNTIME_PROFILE_INVALID');
  }
...
function main(argv = process.argv.slice(2)) {
  ...
  const outputPath = path.resolve(argument(argv, 'out', path.join(candidateRoot, '..', 'a9-win7-34-evidence', 'a9-package-integrity.json')));
  if (contained(candidateRoot, outputPath)) throw new Error('A9_W34_EVIDENCE_INSIDE_CANDIDATE');
  ...
  check('W34PKG-IDENTITY-AUTHORITY-FULL-TREE-ZIP', () => {
  ...
  check('W34PKG-RUNTIME-ABI', () => {
    if (!process.versions.electron || process.versions.electron !== '22.3.27'
      ...
      throw new Error(`A9_W34_RUNTIME_ABI_INVALID:${process.versions.electron}:${process.versions.modules}`);
    }
  });
  const report = {
    schema_version: 1,
    record_id: `A9-16-WIN7-34-PACKAGE-INTEGRITY-${Date.now()}`,
    recorded_at: new Date().toISOString(),
    status: cases.every((item) => item.status === 'PASS') ? 'PASS' : 'FAIL',
    candidate_label: 'WIN7-34',
