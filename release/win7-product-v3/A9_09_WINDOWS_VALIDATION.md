# A9-09 / WIN7-21 增量实机验收

WIN7-21 必须解压到新目录，并保留 WIN7-19 程序目录、候选 ZIP、manifest 与外置证据以便回退。
先执行 `RUN_A9_09_INTEGRITY.cmd <原始ZIP> <外部正式lock> <外部批准清单> <外部批准记录> <独立批准SHA256>`，再以普通用户启动正式
`electron.exe`。证据只能写到候选目录之外。

本轮按 ADR-0097 继承 WIN7-19 未受 D-013 v25 影响的证据；包内
`A9_09_VALIDATION_KIT.json` 中所有 `W21-*` 用例必须由当前候选直接执行：

- 既有 WIN7-19 schema v4 精确 preimage、正式 Electron 启动、迁移前备份与 `quick_check`、规范化后
  PRAGMA 和会话/草稿/元数据事实保持；回滚复验由候选自带 Electron Node runtime 的外置脚本先持有
  SQLite `BEGIN IMMEDIATE` 写锁，再启动未修改的正式 Electron 候选，要求修复事务失败、原库字节不变、
  备份可读并进入 `A9_PERSISTENCE_DIAGNOSTICS / database is locked`；
...
完成后用 `RUN_WIN7_21_REPORT_VERIFY.cmd` 校验报告、候选身份与全部外置证据哈希。只有
A9-13 独审、产品双构建和本轮实机项目全部通过，才能签发新的 `A9_13_WIN7_21_GO_FOR_ALPHA`。
...
Chromium/Electron 许可证、正式 `a9-13-win7-21-input-lock.json`、Shell Preload/Renderer/主进程、Core/Gateway/
...
- `a9-13-win7-21-input-lock.json`：A9-13 新冻结的正式锁，包内副本须逐字节一致；旧 `a9-09-input-lock.json` 仅为历史记录，不能用于 WIN7-21。
...
{
  "schema_version": 1,
  "kind": "WIN7_21_RELEASE_AUTHORITY",
  "status": "APPROVED_FOR_WIN7_21_VALIDATION",
...
签发顺序：用户授权提交修复 → A9-13 独审关闭 → 新 `a9-13-win7-21-input-lock.json` 与源码 G0 冻结 →
两个全新 detached 工作树做产品确定性双构建 → 同一候选预飞行 → 外部批准记录与独立 pin → WIN7-21。
...
```text
node release/win7-product-v3/a9-win7-21-report.cjs init --zip <原始ZIP> --release-manifest <解压目录/release-manifest.json> --kit <解压目录/A9_09_VALIDATION_KIT.json> --formal-input-lock <外部/a9-13-win7-21-input-lock.json> --approval-registry <外部/a9-v25-approved-kits.json> --release-authority <外部/release-authority.json> --release-authority-sha256 <独立批准SHA256>
```

现场 CMD（所有含空格路径均加双引号）：

```text
RUN_A9_09_INTEGRITY.cmd <原始ZIP> <外部正式lock> <外部批准清单> <外部批准记录> <独立批准SHA256>
RUN_WIN7_21_REPORT_VERIFY.cmd <原始ZIP> <报告JSON> <外部证据根> <外部正式lock> <外部批准清单> <外部批准记录> <独立批准SHA256>
```
...
报告必须使用 `schema_version: 2` / `report_kind: WIN7_21_INCREMENTAL_ACCEPTANCE`。8 个 `W17-*`
...
自报的证据集合均不被接受。所有 `W21-*` 只能使用当前候选的直接
