  ...
  const server = http.createServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      if (parsed?.tools?.[0]?.function?.name === 'probe_test_echo') {
        send({ choices: [{ delta: { tool_calls: [{ index: 0, id: 'probe-w23', function: { name: 'probe_test_echo', arguments: '{"message":"probe_ok"}' } }] }, finish_reason: 'tool_calls' }] });
      } else {
...
function createJourneyFixture() {
  ...
  return createFixture((parsed) => {
    ...
    const tools = messages.filter((item) => item.role === 'tool').map((item) => item.name);
    if (turn === 3) {
      if (!tools.includes('read')) return { id: 'w23-r3', note: '正在重新读取文件，确认重启后的会话可以继续。', tool: { name: 'read', args: { path: 'calc.ts' } } };
      return { content: 'second process turn completed.' };
    }
    if (turn === 2) {
      if (!tools.includes('delete')) return { id: 'w23-d1', note: '删除属于高影响操作，等待精确审批。', tool: { name: 'delete', args: { path: 'scratch.tmp', permanent: true } } };
      return { content: 'cleanup denied; nothing executed.' };
    }
    if (!tools.includes('read')) return { id: 'w23-r1', note: '先读取目标文件，再决定最小修改。', tool: { name: 'read', args: { path: 'calc.ts' } } };
    if (!tools.includes('edit')) return { id: 'w23-e1', note: '已确认问题，现在修正计算逻辑。', tool: { name: 'edit', args: { path: 'calc.ts', oldText: 'return a - b;', newText: 'return a + b;' } } };
    if (!tools.includes('shell')) return { id: 'w23-s1', note: '修改已完成，接下来运行目标验证。', tool: { name: 'shell', args: { command: "Write-Output 'smoke-verified'" } } };
...
function createStopFixture(markerPath) {
  ...
  return createFixture((parsed) => {
    ...
    return { id: 'w23-stop', note: '长任务已启动，等待用户停止。', tool: { name: 'shell', args: { command } } };
  });
}
...
async function main() {
  if (process.platform !== 'win32' || process.versions.electron !== '22.3.27'
    ...
    throw new Error(`A9_W23_SMOKE_RUNTIME_INVALID:${process.platform}:${process.versions.electron}:${process.versions.modules}`);
  }
  const candidateRoot = path.resolve(__dirname, '..');
  const electronPath = path.join(candidateRoot, 'electron.exe');
  ...
  const driverPath = path.join(__dirname, 'a9-win7-23-driver.cjs');
  const evidenceRoot = path.resolve(argument('evidence-root', path.join(candidateRoot, '..', 'a9-win7-23-evidence')));
  if (path.relative(candidateRoot, evidenceRoot) === '' || !path.relative(candidateRoot, evidenceRoot).startsWith('..')) {
    throw new Error('A9_W23_SMOKE_EVIDENCE_INSIDE_CANDIDATE');
  }
  ...
  const report = {
    schema_version: 1,
    evidence_kind: 'A9_15_WIN7_23_AUTOMATIC_PRODUCT_SMOKE',
    recorded_at: new Date().toISOString(),
    status: phases.every((item) => item.code === 0) && Object.values(reports).every((item) => item.status === 'PASS')
    ...
    run_root: runRoot,
    fixture_requests: journey.requests.length,
