...
function copyTree(source, destination) {
  ...
  for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
    ...
    else throw new Error(`A9_W32_DRIVER_RUNTIME_SPECIAL_FILE:${from}`);
  }
}
...
function prepareDriverRuntime(candidateRoot, runRoot, sourceDriver) {
  ...
  if (!fs.existsSync(sourceContract)) throw new Error(`A9_W32_DRIVER_CONTRACT_SOURCE_MISSING:${sourceContract}`);
  ...
  if (sha256File(contractTarget) !== contractSha256) throw new Error('A9_W32_DRIVER_CONTRACT_HASH_MISMATCH');
  fs.writeFileSync(path.join(appRoot, 'package.json'), `${JSON.stringify({
    name: 'a9-win7-32-driver', version: '1.0.0', main: 'main.cjs', private: true,
  }, null, 2)}\n`, 'utf8');
  ...
  const server = http.createServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      if (parsed?.tools?.[0]?.function?.name === 'probe_test_echo') {
        send({ choices: [{ delta: { tool_calls: [{ index: 0, id: 'probe-w32', function: { name: 'probe_test_echo', arguments: '{"message":"probe_ok"}' } }] }, finish_reason: 'tool_calls' }] });
      } else {
...
function createJourneyFixture() {
  ...
  return createFixture((parsed) => {
    ...
    if (turn === 5) {
      if (!tools.includes('shell')) return { id: 'w32-f5', note: '执行预期失败的非零退出命令。', tool: { name: 'shell', args: { command: 'exit 3' } } };
      return { content: 'failing shell command observed.' };
    }
    // F3（W28-H02）：可重复工具错误——读取测试专用缺失目标，独立于 Provider 503 与非零退出。
    if (turn === 6) {
      if (!tools.includes('read')) return { id: 'w32-f6', note: '读取缺失的测试专用目标，产生可重复工具错误。', tool: { name: 'read', args: { path: 'missing-fixture-target.ts' } } };
    ...
    if (turn === 7) {
      if (!tools.includes('delete')) return { id: 'w32-a7', note: '高影响删除等待精确审批。', tool: { name: 'delete', args: { path: 'approve-target.tmp', permanent: true } } };
    ...
    if (turn === 8) {
      if (tools.length < BULK_STEPS) return { id: `w32-b8-${tools.length}`, note: '批量只读探查。', tool: { name: 'search', args: { pattern: `probe-${tools.length}-${Date.now() % 100000}` } } };
      return { content: 'bulk history generated and verified.' };
    }
    if (turn === 3) {
      if (!tools.includes('read')) return { id: 'w32-r3', note: '正在重新读取文件，确认重启后的会话可以继续。', tool: { name: 'read', args: { path: 'calc.ts' } } };
      return { content: 'second process turn completed.' };
    }
    if (turn === 2) {
      if (!tools.includes('delete')) return { id: 'w32-d1', note: '删除属于高影响操作，等待精确审批。', tool: { name: 'delete', args: { path: 'scratch.tmp', permanent: true } } };
      return { content: 'cleanup denied; nothing executed.' };
    }
    if (turn === 4) {
      if (!tools.includes('edit')) return { id: 'w32-e4', note: '形成较新修改事实。', tool: { name: 'edit', args: { path: 'calc.ts', oldText: 'return a + b;', newText: 'return a + b; // verified after older failure' } } };
      if (!tools.includes('shell')) return { id: 'w32-s4', note: '验证较新修改。', tool: { name: 'shell', args: { command: "Write-Output 'projection-verified'" } } };
      return { content: 'latest projection verified.' };
    }
    if (!tools.includes('read')) return { id: 'w32-r1', note: '先读取目标文件，再决定最小修改。', tool: { name: 'read', args: { path: 'calc.ts' } } };
    if (!tools.includes('edit')) return { id: 'w32-e1', note: '已确认问题，现在修正计算逻辑。', tool: { name: 'edit', args: { path: 'calc.ts', oldText: 'return a - b;', newText: 'return a + b;' } } };
    if (!tools.includes('shell')) return { id: 'w32-s1', note: '修改已完成，接下来运行目标验证。', tool: { name: 'shell', args: { command: "Write-Output 'smoke-verified'" } } };
...
function createStopFixture(markerPath) {
  ...
  return createFixture((parsed) => {
    ...
    return { id: 'w32-stop', note: '长任务已启动，等待用户停止。', tool: { name: 'shell', args: { command } } };
  });
}
...
async function main() {
  if (process.platform !== 'win32' || process.versions.electron !== '22.3.27'
    ...
    throw new Error(`A9_W32_SMOKE_RUNTIME_INVALID:${process.platform}:${process.versions.electron}:${process.versions.modules}`);
  }
  ...
  const sourceDriver = path.join(__dirname, 'a9-win7-32-driver.cjs');
  const evidenceRoot = path.resolve(argument('evidence-root', path.join(candidateRoot, '..', 'a9-win7-32-evidence')));
  if (path.relative(candidateRoot, evidenceRoot) === '' || !path.relative(candidateRoot, evidenceRoot).startsWith('..')) {
    throw new Error('A9_W32_SMOKE_EVIDENCE_INSIDE_CANDIDATE');
  }
  ...
  record('A9-W32-DRIVER-CONTRACT-CLOSURE',
  ...
  record('A9-W32-DENY-TARGET-SURVIVES-DENIAL',
  ...
  record('A9-W32-APPROVE-TARGET-EXECUTED-AFTER-APPROVAL',
  ...
  record('A9-W32-RETRY-TARGET-BOUND', retryTargetBound,
  ...
  record('A9-W32-DRIVER-PROTOCOL-DECLARED', baseEnv.A9_SMOKE_DRIVER_PROTOCOL === 'projection', baseEnv.A9_SMOKE_DRIVER_PROTOCOL);
  record('A9-W32-FIXTURE-SUPPORTS-PROJECTION-PROTOCOL', failureServed && latestSuccessServed,
    `failure503=${failureServed}; latestSuccess200=${latestSuccessServed}`);
  record('A9-W32-FIXTURE-JOURNEY-STILL-COMPATIBLE', journeyServed, `journey200=${journeyServed}`);
  ...
  record('A9-W32-PROJECTION-EVIDENCE-PUBLISHED',
  ...
  try {
    const reportModule = require('./a9-win7-32-report.cjs');
    const readProjection = (name) => JSON.parse(fs.readFileSync(path.join(evidenceRoot, name), 'utf8'));
    const query = reportModule.parseQueryExport(readProjection('projection-query-export.json'), 'W32-03-INSPECTOR-PERSISTED-RESTART');
    reportModule.parseDomExport(readProjection('projection-dom-export.json'), query, 'W32-03-INSPECTOR-PERSISTED-RESTART', 'restart');
    const evidencePackage = readProjection('projection-evidence.json');
    const outcome = evidencePackage.results['W32-09-LATEST-OUTCOME-PROJECTION'].projection_evidence;
    ...
    const latestTurn = reportModule.latestTerminal(query, 'W32-09-LATEST-OUTCOME-PROJECTION');
  ...
  record('A9-W32-PROJECTION-ARTIFACTS-REPORT-PARSEABLE', projectionParseable, projectionParseDetail);
  ...
  const requiredSmokeAssertionIds = [
    ...
    'A9-W32-DRIVER-PROTOCOL-DECLARED',
    'A9-W32-FIXTURE-SUPPORTS-PROJECTION-PROTOCOL',
    'A9-W32-PROJECTION-EVIDENCE-PUBLISHED',
    'A9-W32-PROJECTION-ARTIFACTS-REPORT-PARSEABLE',
    'A9-W32-RETRY-TARGET-BOUND',
  ];
  ...
  record('A9-W32-REQUIRED-ASSERTIONS-PRESENT', missingRequiredAssertions.length === 0,
    missingRequiredAssertions.length === 0 ? 'ALL_PRESENT' : `MISSING:${missingRequiredAssertions.join(',')}`);
  const cases = [...allCases, ...smokeCases];
  const report = {
    schema_version: 1,
    evidence_kind: 'A9_16_WIN7_32_AUTOMATIC_PRODUCT_SMOKE',
