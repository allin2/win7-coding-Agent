// DERIVED FROM THE FROZEN WIN7-31 ARTIFACT — WIN7-32 G3 real-125%-DPI capacity repair, ADR-0128.
...
// intentionally preserved because WIN7-32 inherits the WIN7-28 acceptance contract.
'use strict';

/**
 * A9-16 / WIN7-32 验收报告器（ADR-0128）。
 *
...
const { selectPhysicalFileSystem, verifyAcceptanceCandidate } = require('./a9-package-integrity-w32.cjs');
const contract = require('./a9-projection-contract.cjs');

const CANDIDATE_LABEL = 'WIN7-32';
const KIT_ID = 'A9-16-WIN7-32-RESPONSIVE-UI-20260915-01';
const REPORT_KIND = 'A9_16_WIN7_32_RESPONSIVE_UI_ACCEPTANCE';
...
const INSPECTOR_CASE = 'W32-03-INSPECTOR-PERSISTED-RESTART';
const PROJECTION_CASE = 'W32-09-LATEST-OUTCOME-PROJECTION';
const PAGING_CASE = 'W32-10-OLDER-EVENT-PAGINATION';
const REAL_PROVIDER_CASE = 'W32-07-REAL-PROVIDER-MULTITOOL';
...
function argumentsOf(argv) {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    assert(item.startsWith('--'), `A9_W32_ARGUMENT_INVALID:${item}`);
    const equal = item.indexOf('=');
    if (equal > 2) values[item.slice(2, equal)] = item.slice(equal + 1);
    else {
      assert(index + 1 < argv.length, `A9_W32_ARGUMENT_VALUE_MISSING:${item}`);
...
function requiredFile(value, label, fileSystem) {
  assert(typeof value === 'string' && value, `A9_W32_${label}_REQUIRED`);
  const filePath = path.resolve(value);
  assert(fileSystem.existsSync(filePath) && fileSystem.statSync(filePath).isFile(), `A9_W32_${label}_NOT_FILE`);
...
function rejectSecrets(value) {
  ...
  assert(!/(?:Bearer\s+[A-Za-z0-9._~+\/-]{8,}|\bsk-[A-Za-z0-9_-]{8,}|-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----)/i.test(serialized), 'A9_W32_REPORT_SECRET_MATERIAL');
  const visit = (item) => {
    if (Array.isArray(item)) return item.forEach(visit);
    if (!plain(item)) return;
    for (const [key, child] of Object.entries(item)) {
      assert(!/^(?:api[_-]?key|apikey|authorization|password|secret|access[_-]?token|refresh[_-]?token|credentials?|private[_-]?key)$/i.test(key), `A9_W32_REPORT_SECRET_FIELD:${key}`);
      visit(child);
    }
  };
  visit(value);
...
function validateEvidence(entries, evidenceRoot, fileSystem, caseId) {
  assert(Array.isArray(entries) && entries.length > 0, `A9_W32_EVIDENCE_REQUIRED:${caseId}`);
  const seen = new Set();
  for (const item of entries) {
    assert(plain(item) && typeof item.path === 'string' && item.path
      && /^[a-f0-9]{64}$/.test(String(item.sha256 || '')) && !seen.has(item.path), `A9_W32_EVIDENCE_ENTRY_INVALID:${caseId}`);
    seen.add(item.path);
    const unresolved = path.resolve(evidenceRoot, item.path);
    assert(fileSystem.existsSync(unresolved) && fileSystem.statSync(unresolved).isFile(), `A9_W32_EVIDENCE_NOT_FILE:${caseId}:${item.path}`);
    const absolute = fileSystem.realpathSync(unresolved);
    assert(contained(evidenceRoot, absolute), `A9_W32_EVIDENCE_ESCAPES_ROOT:${caseId}:${item.path}`);
    assert(sha256(fileSystem.readFileSync(absolute)) === item.sha256, `A9_W32_EVIDENCE_HASH_MISMATCH:${caseId}:${item.path}`);
  }
}
...
function readArtifact(reference, evidence, evidenceRoot, fileSystem, caseId, label) {
  assert(plain(reference) && typeof reference.path === 'string' && /^[a-f0-9]{64}$/.test(reference.sha256 || ''),
    `A9_W32_PROJECTION_ARTIFACT_INVALID:${caseId}:${label}`);
  assert(evidence.some((item) => item.path === reference.path && item.sha256 === reference.sha256),
    `A9_W32_PROJECTION_ARTIFACT_UNBOUND:${caseId}:${label}`);
  ...
  assert(sha256(bytes) === reference.sha256, `A9_W32_PROJECTION_ARTIFACT_HASH_MISMATCH:${caseId}:${label}`);
  let parsed = null;
  try {
    parsed = JSON.parse(bytes.toString('utf8'));
  } catch (_error) {
    throw new Error(`A9_W32_PROJECTION_ATTACHMENT_NOT_JSON:${caseId}:${label}`);
  }
  assert(plain(parsed), `A9_W32_PROJECTION_ATTACHMENT_INVALID:${caseId}:${label}`);
  return parsed;
}
...
function parseQueryExport(exported, caseId) {
  assert(exported.schema_version === QUERY_EXPORT_SCHEMA_VERSION && exported.kind === QUERY_EXPORT_KIND,
    `A9_W32_PROJECTION_QUERY_KIND_INVALID:${caseId}`);
  assert(typeof exported.conversation_id === 'string' && exported.conversation_id.length > 0,
    `A9_W32_PROJECTION_CONVERSATION_REQUIRED:${caseId}`);
  assert(Array.isArray(exported.events) && exported.events.length > 0, `A9_W32_PROJECTION_QUERY_EMPTY:${caseId}`);
  // F4：查询附件必须保存真实查询事实（limit/beforeEventId/hasMore/返回范围），不得硬编码。
  assert(Array.isArray(exported.pages) && exported.pages.length > 0, `A9_W32_PROJECTION_QUERY_PAGES_REQUIRED:${caseId}`);
  for (const page of exported.pages) {
    ...
    `A9_W32_PROJECTION_QUERY_PAGE_INVALID:${caseId}`);
  }
  const bound = (value, cap) => {
    assert(value === null || value === undefined || (typeof value === 'string' && value.length <= cap),
      `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
  };
  const events = [];
  const seen = new Set();
  for (const event of exported.events) {
    assert(plain(event) && isEventId(event.event_id) && !seen.has(event.event_id), `A9_W32_PROJECTION_EVENT_ID_INVALID:${caseId}`);
    assert(event.turn_id === null || (typeof event.turn_id === 'string' && event.turn_id.length > 0),
      `A9_W32_PROJECTION_EVENT_TURN_INVALID:${caseId}`);
    assert(typeof event.type === 'string' && event.type.length > 0, `A9_W32_PROJECTION_EVENT_TYPE_INVALID:${caseId}`);
    assert(event.outcome === undefined || event.outcome === null || typeof event.outcome === 'string', `A9_W32_PROJECTION_EVENT_OUTCOME_INVALID:${caseId}`);
    assert(event.verification === undefined || event.verification === null || typeof event.verification === 'string', `A9_W32_PROJECTION_EVENT_VERIFICATION_INVALID:${caseId}`);
    // F2/时间：重建显示文本与显示时间所需的脱敏事实必须具备且必须有界。
    assert(Number.isSafeInteger(event.timestamp_ms), `A9_W32_PROJECTION_EVENT_TIMESTAMP_REQUIRED:${caseId}`);
    const display = event.display;
    assert(plain(display), `A9_W32_PROJECTION_EVENT_DISPLAY_REQUIRED:${caseId}`);
    ...
    assert(display.denied === undefined || typeof display.denied === 'boolean', `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
    assert(display.has_error === undefined || typeof display.has_error === 'boolean', `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
    assert(display.shell_has_exit_code === undefined || typeof display.shell_has_exit_code === 'boolean', `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
    assert(display.shell_exit_code === undefined || display.shell_exit_code === null || Number.isSafeInteger(display.shell_exit_code), `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
    assert(display.step === undefined || display.step === null || Number.isSafeInteger(display.step), `A9_W32_PROJECTION_DISPLAY_BOUND_INVALID:${caseId}`);
    assert(plain(display.args), `A9_W32_PROJECTION_DISPLAY_ARGS_REQUIRED:${caseId}`);
    ...
    assert(!previous || event.event_id > previous.event_id, `A9_W32_PROJECTION_QUERY_ORDER_INVALID:${caseId}`);
...
function parseDomExport(exported, query, caseId, stage) {
  assert(exported.schema_version === DOM_EXPORT_SCHEMA_VERSION && exported.kind === DOM_EXPORT_KIND,
    `A9_W32_PROJECTION_DOM_KIND_INVALID:${caseId}:${stage}`);
  assert(typeof exported.stage === 'string' && STAGES.includes(exported.stage) && exported.stage === stage,
    `A9_W32_PROJECTION_DOM_STAGE_MISMATCH:${caseId}:${stage}:${exported.stage}`);
  assert(exported.conversation_id === query.conversationId, `A9_W32_PROJECTION_CONVERSATION_MISMATCH:${caseId}:${stage}`);
  assert(typeof exported.displayed_outcome === 'string' && exported.displayed_outcome.length > 0,
    `A9_W32_PROJECTION_DOM_OUTCOME_MISSING:${caseId}:${stage}`);
  ...
  `A9_W32_PROJECTION_DOM_LATEST_TURN_INVALID:${caseId}:${stage}`);
  ...
  assert(plain(exported.time_baseline) && contract.deriveTimeBaseline(exported.time_baseline) !== null,
    `A9_W32_PROJECTION_DOM_TIME_BASELINE_INVALID:${caseId}:${stage}`);
  assert(plain(exported.display_range) && exported.display_range.rule === INSPECTOR_DISPLAY_RULE
    && exported.display_range.max_rows === INSPECTOR_DISPLAY_ROWS, `A9_W32_PROJECTION_DISPLAY_RULE_INVALID:${caseId}:${stage}`);
  assert(Array.isArray(exported.rows), `A9_W32_PROJECTION_DOM_ROWS_REQUIRED:${caseId}:${stage}`);
  // 正向判定与全部负向变异共用同一函数：身份、顺序、去重、内容或时间不符一律拒绝。
  assert(contract.rowsMatchQuery(exported.rows, query.events, exported.time_baseline),
    `A9_W32_PROJECTION_DOM_ROWS_MISMATCH:${caseId}:${stage}`);
  assert(exported.display_range.rows_total === expectedRange(query).length, `A9_W32_PROJECTION_DISPLAY_TOTAL_INVALID:${caseId}:${stage}`);
  return {
...
function latestTerminal(query, caseId) {
  ...
  assert(terminals.length > 0, `A9_W32_PROJECTION_TERMINAL_MISSING:${caseId}`);
  const newest = terminals.reduce((a, b) => (b.event_id > a.event_id ? b : a));
  assert(terminalFacts(newest) !== null, `A9_W32_PROJECTION_TERMINAL_OUTCOME_MISSING:${caseId}`);
  const ordered = terminals.every((event, index) => index === 0 || event.event_id > terminals[index - 1].event_id);
  assert(ordered, `A9_W32_PROJECTION_TERMINAL_ORDER_INVALID:${caseId}`);
  return newest;
}

function expectedDisplayed(terminal) {
  const facts = terminalFacts(terminal);
  assert(facts, `A9_W32_PROJECTION_TERMINAL_OUTCOME_MISSING:${terminal && terminal.event_id}`);
...
function expectedDisplayed(event) {
  ...
  assert(facts, `A9_W32_PROJECTION_TERMINAL_OUTCOME_MISSING:${event && event.event_id}`);
  return `${facts.outcome} · ${facts.verification}`;
}
...
function validateInspectorProjection(proof, evidence, evidenceRoot, fileSystem, caseId) {
  ...
  assert(query.events.some((event) => event.turn_id === null), `A9_W32_PROJECTION_SESSION_EVENTS_MISSING:${caseId}`);
  assert(query.events.some((event) => event.type === 'tool_start') && query.events.some((event) => event.type === 'tool_end'),
    `A9_W32_PROJECTION_TOOL_EVENTS_MISSING:${caseId}`);
  const terminal = latestTerminal(query, caseId);
  assert(dom.rows.some((row) => row.event_id === terminal.event_id), `A9_W32_PROJECTION_TERMINAL_ROW_MISSING:${caseId}`);
  // F1：DOM 附件的实际显示结果与最新持久化 turn 身份必须与查询最新终态一致。
  const expectedDisplay = expectedDisplayed(terminal);
  assert(dom.displayedOutcome === expectedDisplay, `A9_W32_PROJECTION_DOM_OUTCOME_MISMATCH:${caseId}:restart`);
  assert((dom.latestPersistedTurnId || null) === (terminal.turn_id || null),
    `A9_W32_PROJECTION_DOM_LATEST_TURN_MISMATCH:${caseId}:restart`);
  // 切换会话后不得残留上一会话内容，且切回后逐行复原。
  assert(plain(proof.session_switch), `A9_W32_PROJECTION_SESSION_SWITCH_REQUIRED:${caseId}`);
  ...
  assert(other.schema_version === DOM_EXPORT_SCHEMA_VERSION && other.kind === DOM_EXPORT_KIND
    && other.stage === 'other_conversation', `A9_W32_PROJECTION_DOM_KIND_INVALID:${caseId}:other`);
  assert(typeof other.conversation_id === 'string' && other.conversation_id.length > 0
    && other.conversation_id !== query.conversationId, `A9_W32_PROJECTION_SESSION_SWITCH_SAME_CONVERSATION:${caseId}`);
  // W28-H05：其他会话 DOM 必须是非空、身份有效且与原会话事件 ID 无交集（共享契约同一判定，
  // 方向为"无交集"；空会话不能单独证明隔离）。
  assert(!contract.sessionResidueViolation(other.rows, query.ids),
    `A9_W32_PROJECTION_CROSS_SESSION_RESIDUE:${caseId}`);
  ...
  assert(canonical(resumed.rows.map((row) => row.event_id)) === canonical(dom.rows.map((row) => row.event_id)),
    `A9_W32_PROJECTION_RESUME_MISMATCH:${caseId}`);
  assert(resumed.displayedOutcome === expectedDisplay, `A9_W32_PROJECTION_DOM_OUTCOME_MISMATCH:${caseId}:resume`);
  assert((resumed.latestPersistedTurnId || null) === (terminal.turn_id || null),
    `A9_W32_PROJECTION_DOM_LATEST_TURN_MISMATCH:${caseId}:resume`);
...
function validateOutcomeProjection(proof, evidence, evidenceRoot, fileSystem, caseId) {
  ...
  assert(plain(older) && isEventId(older.event_id) && typeof older.turn_id === 'string' && older.turn_id.length > 0,
    `A9_W32_PROJECTION_OLDER_BINDING_INVALID:${caseId}`);
  assert(plain(newer) && isEventId(newer.event_id) && typeof newer.turn_id === 'string' && newer.turn_id.length > 0,
    `A9_W32_PROJECTION_NEWER_BINDING_INVALID:${caseId}`);
  assert(older.turn_id !== newer.turn_id, `A9_W32_PROJECTION_TURN_IDENTITY_COLLISION:${caseId}`);
  assert(older.event_id < newer.event_id, `A9_W32_PROJECTION_EVENT_IDENTITY_ORDER_INVALID:${caseId}`);
  ...
  `A9_W32_PROJECTION_OLDER_QUERY_MISMATCH:${caseId}`);
  ...
  `A9_W32_PROJECTION_NEWER_QUERY_MISMATCH:${caseId}`);
  const terminal = latestTerminal(query, caseId);
  assert(terminal.event_id === newer.event_id, `A9_W32_PROJECTION_STALE_GLOBAL_OUTCOME:${caseId}`);
  const expected = expectedDisplayed(terminal);
  assert(expected === DISPLAYED_OUTCOME, `A9_W32_PROJECTION_LATEST_NOT_VERIFIED:${caseId}`);
  assert(typeof proof.older_load_mode === 'string' && OLDER_LOAD_MODES.includes(proof.older_load_mode),
    `A9_W32_PROJECTION_OLDER_LOAD_MODE_INVALID:${caseId}`);
  assert(query.ids.length <= INSPECTOR_DISPLAY_ROWS || proof.older_load_mode === 'CLICKED_LOAD_MORE',
    `A9_W32_PROJECTION_RANGE_TOO_LARGE:${caseId}`);
  ...
  for (const [label, dom] of [['restart', afterRestart], ['older_load', afterOlderLoad]]) {
    assert(dom.displayedOutcome === expected, `A9_W32_PROJECTION_DOM_OUTCOME_MISMATCH:${caseId}:${label}`);
    assert((dom.latestPersistedTurnId || null) === (newer.turn_id || null),
      `A9_W32_PROJECTION_DOM_LATEST_TURN_MISMATCH:${caseId}:${label}`);
  }
  assert(proof.restart_displayed_outcome === expected && proof.restart_displayed_outcome === afterRestart.displayedOutcome,
    `A9_W32_PROJECTION_RESTART_OUTCOME_MISMATCH:${caseId}`);
...
function validatePagingProjection(proof, evidence, evidenceRoot, fileSystem, caseId) {
  ...
  `A9_W32_PROJECTION_PAGING_OLDER_BINDING_MISMATCH:${caseId}`);
  ...
  assert(queryOlder && queryOlder.turn_id === older.turn_id && queryOlder.type === 'turn_failed',
    `A9_W32_PROJECTION_PAGING_OLDER_QUERY_MISMATCH:${caseId}`);
  // 用同一共享契约独立重算分页链，不信任 paging.ok 摘要。
  const verdict = contract.validatePagingChain(paging);
  assert(verdict.ok === true,
    `A9_W32_PROJECTION_PAGING_CHAIN_INVALID:${caseId}:${(verdict.violations || []).join(',')}`);
  ...
  if (paging.first_screen && Array.isArray(paging.first_screen.event_ids)) {
    ...
    assert(canonical(paging.first_screen.event_ids) === canonical(expectedFirstIds),
      `A9_W32_PROJECTION_PAGING_FIRST_SCREEN_MEMBERS_MISMATCH:${caseId}`);
  }
    ...
    for (const page of paging.pages) {
      if (page && page.request && page.response && Array.isArray(page.response.event_ids)) {
        ...
        assert(canonical(page.response.event_ids) === canonical(expectedIds),
          `A9_W32_PROJECTION_PAGING_MEMBERS_MISMATCH:${caseId}`);
        assert(page.response.count === expectedIds.length,
          `A9_W32_PROJECTION_PAGING_COUNT_MISMATCH:${caseId}`);
        ...
        assert(Boolean(page.response.has_more) === hasMoreExpected,
          `A9_W32_PROJECTION_PAGING_HAS_MORE_MISMATCH:${caseId}`);
      }
      for (const terminal of (page.response && page.response.terminal_events) || []) {
        ...
        if (matchingQuery) {
          assert((matchingQuery.turn_id || null) === (terminal.turn_id || null) && matchingQuery.type === terminal.type,
            `A9_W32_PROJECTION_PAGING_TERMINAL_QUERY_MISMATCH:${caseId}`);
        }
      }
    }
  }
  // 查询附件必须记录真实游标与 hasMore，而不是硬编码 has_more=false。
  assert(query.pages.some((page) => page.before_event_id !== null), `A9_W32_PROJECTION_PAGES_CURSOR_NOT_RECORDED:${caseId}`);
  assert(query.pages.some((page) => page.before_event_id === null && page.has_more === true),
    `A9_W32_PROJECTION_PAGES_FIRST_HAS_MORE_INVALID:${caseId}`);
  return { query, paging };
}

function validateProjectionEvidence(proof, evidence, evidenceRoot, fileSystem, caseId) {
  assert(plain(proof), `A9_W32_PROJECTION_EVIDENCE_REQUIRED:${caseId}`);
...
function validateExecution(execution, validationCase, identity, evidenceRoot, fileSystem) {
  assert(plain(execution) && canonical(execution.candidate) === canonical(identity), `A9_W32_EXECUTION_CANDIDATE_MISMATCH:${validationCase.case_id}`);
  assert(typeof execution.run_id === 'string' && /^[A-Za-z0-9._-]{8,128}$/.test(execution.run_id), `A9_W32_RUN_ID_INVALID:${validationCase.case_id}`);
  ...
  `A9_W32_ENVIRONMENT_INVALID:${validationCase.case_id}`);
  assert(Array.isArray(execution.assertions), `A9_W32_ASSERTIONS_REQUIRED:${validationCase.case_id}`);
  ...
  for (const assertion of execution.assertions) {
    assert(plain(assertion) && expected.has(assertion.assertion_id) && !seen.has(assertion.assertion_id)
      && assertion.status === 'PASS', `A9_W32_ASSERTION_INVALID:${validationCase.case_id}`);
    seen.add(assertion.assertion_id);
  }
  assert(seen.size === expected.size, `A9_W32_ASSERTIONS_MISSING:${validationCase.case_id}`);
  ...
  if (validationCase.case_id === REAL_PROVIDER_CASE) {
    assert(execution.provider_kind === 'REAL_NON_FIXTURE' && execution.provider_probe === 'tool_calling', 'A9_W32_REAL_PROVIDER_PROOF_INVALID');
  }
}
...
function verifyReport(report, kit, identity, evidenceRoot, fileSystem) {
  ...
  'A9_W32_KIT_INVALID');
  // ADR-0128：A9-16 UI 子集用例为签发前置；缺任一即拒绝。
  for (const caseId of [...OUTCOME_CASES, PAGING_CASE, 'W32-04-APPROVAL-FAILURE-ORDER',
    'W32-11-LEFT-PANE-DENSITY-AND-CAPACITY',
    'W32-12-DESKTOP-FOUR-STATE-DOM-KEEPALIVE',
    'W32-13-BREAKPOINT-CONVERGENCE-REGRESSION',
    'W32-14-KEYBOARD-AND-FOCUS-CONTRACT',
    'W32-15-REAL-125-PERCENT-DPI-LAYOUT']) {
    assert(kit.required_cases.some((item) => item.case_id === caseId && Array.isArray(item.assertions) && item.assertions.length > 0),
      `A9_W32_KIT_CASE_MISSING:${caseId}`);
  }
  assert(plain(report) && report.schema_version === 1 && report.report_kind === REPORT_KIND, 'A9_W32_REPORT_SCHEMA_INVALID');
  assert(canonical(report.candidate) === canonical(identity), 'A9_W32_CANDIDATE_BINDING_MISMATCH');
  assert(Array.isArray(report.results) && report.results.length === kit.required_cases.length, 'A9_W32_RESULT_COUNT_INVALID');
  const expected = new Map(kit.required_cases.map((item) => [item.case_id, item]));
  const seen = new Set();
  for (const result of report.results) {
    assert(plain(result) && expected.has(result.case_id) && !seen.has(result.case_id), `A9_W32_CASE_ID_INVALID:${result && result.case_id}`);
    seen.add(result.case_id);
    assert(['PASS', 'FAIL', 'NOT_PERFORMED', 'EVIDENCE_PENDING'].includes(result.status), `A9_W32_CASE_STATUS_INVALID:${result.case_id}`);
    assert(Array.isArray(result.executions), `A9_W32_EXECUTIONS_REQUIRED:${result.case_id}`);
    if (result.status === 'PASS') {
      assert(result.executions.length > 0, `A9_W32_PASS_WITHOUT_EXECUTION:${result.case_id}`);
  ...
  assert(seen.size === expected.size, 'A9_W32_CASE_SET_INCOMPLETE');
  ...
  assert(report.status === expectedStatus, 'A9_W32_OVERALL_STATUS_INVALID');
  return {
...
function main(argv = process.argv.slice(2)) {
  ...
  assert(options.command === 'verify', 'A9_W32_COMMAND_INVALID');
  const reportPath = requiredFile(options.report, 'REPORT', fileSystem);
  assert(typeof options['evidence-root'] === 'string' && options['evidence-root'], 'A9_W32_EVIDENCE_ROOT_REQUIRED');
