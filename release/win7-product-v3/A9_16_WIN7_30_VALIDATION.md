# A9-16 / WIN7-30 响应式工作台 UI 子集实机验收

WIN7-30 是 A9-16 §4 U01–U07（左侧对话区与左右栏自适应）的**修正候选**，决策记录为 ADR-0126；
它换发自被判定为构建缺陷的 WIN7-29（见下节），WIN7-29 的冻结身份与证据作为失败构建留档。
不得覆盖、改名或改判 WIN7-22～WIN7-29 任何候选、证据与结论。
...
## WIN7-29 缺陷与 WIN7-30 换发

WIN7-29 冻结后，候选外预检发现其包内 `validation/a9-package-integrity-w29.cjs` 有 5 处字面量
...
`approved.status`。以符合本文件契约的候选外 authority 调用该脚本，实测以
`A9_W29_INPUT_LOCK_CONTRACT_INVALID` 被拒——**候选自带的校验器拒绝候选自身携带的 input lock**，
Win7 实机验收无法通过第一条命令；且 provenance 校验先于 authority 校验失败，不存在可用的标签组合。
该文件被 `release-manifest.json` 哈希绑定并随 ZIP 发布，包外补丁不可行。

因此按既有修复先例换发 WIN7-30：修正范围严格限于上述字面量，使其与
`a9-16-win7-30-input-lock.json` 逐项一致，不扩大候选范围、不改变用例集、不弱化任何断言。
WIN7-29 的冻结身份（commit `4bdf87b`、ZIP `a69d92c4…`、manifest `46f11c0d…`）**保留为失败构建
证据，不得删除、改写或复用其哈希改判**。同时新增残留守卫：回归测试断言派生脚本的
gates/provenance/authority 字面量必须与 input lock 逐项一致，构建期扫描覆盖下划线形式
（`WIN7_28_`、`APPROVED_FOR_WIN7_28_`）等连字符守卫覆盖不到的形式。
...
3. 在可信开发机先运行仓库内 verifier 预检。Win7 上先以普通用户、非提升令牌运行
   `RUN_A9_16_W30_INTEGRITY.cmd`，成功后才可启动正式 `electron.exe`。
...
继承 WIN7-28 合同（`W30-01`～`W30-10`，含投影 `W30-09` 与分页 `W30-10`）并新增 5 项 UI 用例：

- `W30-11-LEFT-PANE-DENSITY-AND-CAPACITY`（U01/U02）：三段式分区不重叠；每行单行不折行；
  **最坏形态（2 组头 + 归档区）下至少 4 条对话行完整可见**；行高保持 36px，容量靠压缩固定
  chrome 而非缩小行高达成；组头按 `activity` 字段。
- `W30-12-DESKTOP-FOUR-STATE-DOM-KEEPALIVE`（U03/U04/U05）：四态均可到达且状态类正确；
...
{
  "schema_version": 1,
  "kind": "WIN7_30_RELEASE_AUTHORITY",
  "status": "APPROVED_FOR_WIN7_30_VALIDATION",
  "formal_input_lock_sha256": "<a9-16-win7-30-input-lock.json SHA-256>",
  "approval_registry": {
    "commit": "<批准清单提交>",
...
```text
RUN_A9_16_W30_INTEGRITY.cmd <原始ZIP> <外部正式lock> <外部批准清单> <外部批准记录> <独立批准记录SHA256>

set "ELECTRON_RUN_AS_NODE=1"
electron.exe validation\a9-win7-30-smoke.cjs --evidence-root=<候选外证据目录>
...
`electron.exe`，按 `A9_16_VALIDATION_KIT.json` 完成 15 项用例，其中 `W30-11`～`W30-15` 必须提供
真实 1366×768 × 125% DPI 下的截图与几何量测。最后生成报告模板：

```text
set "ELECTRON_RUN_AS_NODE=1"
electron.exe validation\a9-win7-30-report.cjs init --kit A9_16_VALIDATION_KIT.json --release-manifest release-manifest.json --zip <原始ZIP> --formal-input-lock <外部正式lock> --approval-registry <外部批准清单> --release-authority <外部批准记录> --release-authority-sha256 <独立批准SHA256> > ..\a9-win7-30-evidence\report-template.json
set "ELECTRON_RUN_AS_NODE="
```

填入实际证据后用 `RUN_WIN7_30_REPORT_VERIFY.cmd` 校验。每个 PASS 必须绑定同一候选身份、普通用户
...
**15 项用例缺一即不得签发**：包完整性与报告器都会显式要求 `W30-11`～`W30-15` 存在。
