...
const result = spawnSync('npm', ['audit', '--package-lock-only', '--json', '--registry=https://registry.npmjs.org'], {
  cwd: path.join(kitRoot, 'tooling'), encoding: 'utf8', maxBuffer: 32 * 1024 * 1024
});
...
report._project_capture = {
  ...
  scope: 'A6 Win10 build-host npm tooling only; runtime findings are classified separately',
