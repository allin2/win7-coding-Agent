...
"%ELECTRON%" "%ROOT%\acceptance\a5\a5-electron-main.js" --t05-only --acceptance-id "%ID%" --out "%ROOT%\evidence" --helper "%ROOT%\spike02_helper.exe" --acceptance-root "%ACCEPTANCE_ROOT%" --per-run-root "%ROOT%\t05-work" --lease-id "%LEASE_ID%" --source-commit "%SOURCE_COMMIT%" --package-manifest-sha256 "%MANIFEST_SHA256%" > "%ROOT%\harness-stdout.txt" 2> "%ROOT%\harness-stderr.txt"
