...
> 当前仓库内 r4 closure 已因 v25 独立复审撤销，状态为
> `REVOKED_PENDING_NEW_COMMITTED_BUILD_KIT`。必须先提交全部修复，更新 `SOURCE_COMMIT*`，运行
> `prepare-kit.cjs` 生成新 revision，封装 ZIP 并在产品批准清单预登记精确哈希；不得从当前未提交目录构建。
